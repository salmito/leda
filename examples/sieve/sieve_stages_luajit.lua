-- sieve_stages_luajit.lua
-- Count the number of prime numbers between 1 and a limit N using
-- the sieve of Eratosthenes with blocks programmed with stages compatible with
-- luajit (without using string.dump)
-- typical usage: lua -e "N=500 BLOCK=100" sieve_stages_luajit.lua"
...
local prime=leda.stage{"Sieve",
	handler=[[
		require 'math'
		local args={...} local from,to=args[1],args[2]
...
sieve:run{maxpar=maxpar or leda.kernel.cpu(),controller=leda.controller.interactive.get(
(maxpar or kernel.cpu())+1)}
